...
def main():
    ...
    if not force:
        confirm = input(f'Deleting L3Out node profile {name}. Continue? [no|yes]')
        if confirm != 'yes':
            print('Aborting...')
            sys.exit(0)
    if not deleteL3OutNodeProfile(ip = apic_ip, token = token, cookies = cookies, name = name, l3out = l3out_name, tenant = tenant):
        logging.error('failed to delete L3Out node profile')
        sys.exit(1)

    # Checking if L3Out is empty
    total, l3outs_nodes = getL3OutNodeProfiles(ip = apic_ip, token = token, cookies = cookies, tenant = tenant, l3out = l3out_name)
    print(total)
    if total is 0:
        logging.error(f'L3OUT {l3out_name} is empty, traffic is blackholed for L3 Bridge Domains')
