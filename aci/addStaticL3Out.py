...
import sys
import yaml
from functions import addL3OutNodeInterfaceProfile
from functions import addL3OutNodeProfile
from functions import addStaticL3Out
from functions import addStaticL3OutConfiguredNodes
from functions import addStaticL3OutSVI
from functions import bindBDtoL3Out
from functions import getBDs
from functions import getL3OutNodeInterfaceProfiles
from functions import getL3OutNodeProfiles
from functions import getL3Outs
from functions import getLeafID
from functions import getPathFromLeafName
from functions import getPathFromLeafPort
from functions import getStaticL3OutConfiguredNodes
from functions import getStaticL3OutSVI
...
def main():
    ...
    # Reading options
    try:
        opts, args = getopt.getopt(sys.argv[1:], "vfbd:n:t:m:i:V:I:l:")
    ...
    for opt, arg in opts:
        if opt == "-v":
            ...
            try:
                ...
                logger.error(
                    "cannot parse Leaf Name, IP and port or policy group"
                )
                sys.exit(1)
        else:
    ...
    l3out_interface_description = (
        f"Logical (SVI) and physical (single/PC/vPC) interfaces facing {name}"
    )
    if not description:
    ...
    if total is 0 or force:
        ...
        if not addStaticL3Out(
            ip=apic_ip,
            token=token,
            cookies=cookies,
            name=l3out_name,
            description=l3out_description,
            tenant=tenant,
            vrf=tenant,
            domain=domain_l3,
        ):
    ...
    total, l3outs_interfaces = getL3OutNodeInterfaceProfiles(
        ip=apic_ip,
        token=token,
        cookies=cookies,
        tenant=tenant,
    ...
    if total is 0 or force:
        # Adding interface profile
        if not addL3OutNodeInterfaceProfile(
            ip=apic_ip,
            token=token,
            cookies=cookies,
            tenant=tenant,
            l3out=l3out_name,
            name="ports",
            description=l3out_interface_description,
            node=name,
        ):
    ...
    for leaf in leafs:
        leaf_id = getLeafID(
            ip=apic_ip, token=token, cookies=cookies, name=leaf["name"]
        )
        vip_mac_address = "00:22:BD:00:0{}:{}".format(
            str(leaf_id)[0], str(leaf_id)[1:3]
        )
        router_id = f"192.168.20.{leaf_id}"

        if "/" in leaf["port"]:
            # This is a single port (i.e. 1/25)
            port_path = getPathFromLeafPort(
                ip=apic_ip,
                token=token,
                cookies=cookies,
                name=leaf["name"],
            ...
            if not port_path:
                logging.error(
                    f'Port {leaf["port"]} not found in Leaf {leaf["name"]}'
                )
                sys.exit(1)
            # Checking if SVI exists
            total, svis = getStaticL3OutSVI(
                ip=apic_ip,
                token=token,
                cookies=cookies,
                tenant=tenant,
                l3out=l3out_name,
                node_name=name,
                interface_name="ports",
                path=port_path,
            )
            if total is 0:
                # Adding the SVI
                if not addStaticL3OutSVI(
                    ip=apic_ip,
                    token=token,
                    cookies=cookies,
                    tenant=tenant,
                    path=port_path,
                    vip_mac_address=vip_mac_address,
                    vlan=vlan,
                    leaf_ip=leaf["ip"],
                    vip_ip_address=vip_ip_address,
                    mode=mode,
                    l3out=l3out_name,
                    node_name=name,
                    name="ports",
            ...
            if total is 0:
                ...
                if not addStaticL3OutConfiguredNodes(
                    ip=apic_ip,
                    token=token,
                    cookies=cookies,
                    tenant=tenant,
                    ...
                    logging.error(
                        f"cannot add physical path {node_path} for L3Out"
                    )
                    sys.exit(1)

    if bind:
        total, bds = getBDs(
            ip=apic_ip, token=token, cookies=cookies, tenant=tenant
        )
        for bd in bds:
            has_subnet = False
            has_l3out = False
            try:
                ...
                logging.debug("bridge domain has no children")
                continue
