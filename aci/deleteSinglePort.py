...
import time
import yaml
from functions import deleteInterfaceProfile
from functions import deleteInterfaceSelectorBlock
from functions import getEPGsFromPath
from functions import getInterfaceProfileFromPortAndLeaf
...
from functions import getPortCfgFromPath
from functions import getSwitchProfilesFromInterfaceProfile
from functions import login
from functions import unbindSwitchProfileFromInterfaceProfile
...
def main():
    ...
    path = getPathFromLeafPort(
        ip=apic_ip, token=token, cookies=cookies, name=leaf, port=port, fex=fex
    )
    if not path:
    ...
    if delete and not fex:
        ...
        if interface_profile:
            ...
            if not force:
                confirm = input(  # nosec
                    f"Unbinding profile {interface_profile} from leaf {leaf}. Continue? [no|yes]"
                )
                if confirm != "yes":
                    print("Aborting...")
                    sys.exit(0)
