...
import sys
import yaml
from functions import addAppProfile
from functions import addTenant
from functions import addVRF
from functions import getTenants
...
def main():
    ...
    if not addAppProfile(
        ip=apic_ip,
        token=token,
        cookies=cookies,
        name=name,
        description=f"Default application profile for tenant {name}",
    ):
    ...
    if not addVRF(
        ...
        cookies=cookies,
        name=name,
        description=f"Default unenforced VRF for tenant {name}",
    ):
