...
import sys
import yaml
from functions import addInterfacePolicyGroup
...
def usage():
    ...
    print(
        "  -F STRING  Fex Profile Name (i.e. FEX_Leaf102:Fex101, must be repeated with -t)"
    )
...
def main():
    ...
    if not po_type:
        ...
        if total is 0 or force:
            ...
            if not addInterfacePolicyGroup(
                ip=apic_ip,
                token=token,
                cookies=cookies,
                name=group,
    ...
    for fex_profile_name in fex_profile_names:
        ...
        if total is 0:
            ...
            logging.warning(
                f"port {port} is already configured on Fex {fex_profile_name}"
            )
            continue
