...
def main():
    ...
    if total > 1:
        ...
        if not bindSwitchProfileToInterfaceProfile(ip = apic_ip, token = token, cookies = cookies, name = leaf, interface_profile = name):
            logging.error(f'failed to bind switch profile {leaf} to interface profile {name}')
            sys.exit(1)
