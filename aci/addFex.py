...
import sys
import yaml
from functions import addFexProfile
...
from functions import getFexProfile
...
def main():
    ...
    total, fex_profiles = getFexProfile(
        ip=apic_ip, token=token, cookies=cookies, name=fex_profile_name
    )
    if total is 0 or force:
        # Adding interface selector block
        if not addFexProfile(
            ip=apic_ip,
            token=token,
            cookies=cookies,
    ...
    total, interface_profiles = getInterfaceProfiles(
        ip=apic_ip, token=token, cookies=cookies, name=leaf_profile_name
    )
    if total is 0 or force:
        ...
        if not addInterfaceProfile(
            ...
            description=f"Port-Channel connected to {fex_profile_name}",
        ):
            logging.error(
                f"failed to create interface profile {leaf_profile_name}"
            )
            sys.exit(1)
    ...
    if total > 1:
        logging.error(
            f"Fex profile {name} is bound to multiple switch profiles"
        )
    elif total == 1:
        ...
        if leaf_profile != connected_leaf:
            logging.error(
                f"Fex profile {name} is bound to a different switch profile {connected_leaf}"
            )
            sys.exit(1)
    else:
        ...
        if not bindSwitchProfileToInterfaceProfile(
            ...
            cookies=cookies,
            name=leaf_profile,
            interface_profile=leaf_profile_name,
        ):
            logging.error(
                f"failed to bind switch profile {leaf_profile_name} to fex profile {name}"
            )
            sys.exit(1)
    ...
    if total is 0 or force:
        ...
        if not addInterfaceSelector(
            ...
            fex_id=id,
            fex_profile_name=fex_profile_name,
        ):
            logging.error(
                f"failed to create fex interface selector {leaf_profile_name}"
            )
            sys.exit(1)
    ...
    for port in ports:
        ...
        if total is 0:
            ...
            if not addInterfaceSelectorBlock(
                ...
                selector="fex_ports",
                name=port,
                description=f"{fex_profile_name} from port {port}",
            ):
                logging.error(
                    f"failed to create interface selector block with {port}"
                )
                sys.exit(1)
