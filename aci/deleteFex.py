...
import sys
import yaml
from functions import deleteFexProfile
...
def main():
    ...
    total, unused = getInterfaceSelectorBlocks(
        ip=apic_ip,
        token=token,
        cookies=cookies,
    ...
    if not force:
        confirm = input(  # nosec
            f"Unbinding {leaf_profile_name} from {leaf_profile}. Continue? [no|yes]"
        )
        if confirm != "yes":
            print("Aborting...")
            sys.exit(0)
    ...
    if not force:
        confirm = input(  # nosec
            f"Deleting interface profile {leaf_profile_name}. Continue? [no|yes]"
        )
        if confirm != "yes":
            print("Aborting...")
            sys.exit(0)
    ...
    if not force:
        confirm = input(  # nosec
            f"Deleting FEX profile {fex_profile_name}. Continue? [no|yes]"
        )
        if confirm != "yes":
            print("Aborting...")
            sys.exit(0)
    if not deleteFexProfile(
        ip=apic_ip, token=token, cookies=cookies, name=fex_profile_name
    ):
