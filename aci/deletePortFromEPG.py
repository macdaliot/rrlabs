...
import sys
import yaml
from function import deletePathFromEPG
from function import getEPGs
from function import getPathFromEPG
...
def main():
    ...
    if port_type == "single":
        ...
        path = getPathFromPolicyGroup(
            ip=apic_ip, token=token, cookies=cookies, name=group
        )
        if not path:
    ...
    # Finding EPG
    total, epgs = getEPGs(
        ip=apic_ip, token=token, cookies=cookies, tenant=tenant, app=tenant
    )
    if total > 0:
        for epg in epgs:
    ...
    if total == 1:
        if not force:
            confirm = input(  # nosec
                f"Deleting path {path} from EPG {epg_name}. Continue? [no|yes]"
            )
            if confirm != "yes":
                print("Aborting...")
                sys.exit(0)
        # Deleting the path from the EPG
        if not deletePathFromEPG(
            ip=apic_ip,
            token=token,
            cookies=cookies,
            tenant=tenant,
