...
import time
import yaml
from function import deleteInterfaceProfile
from function import deleteInterfaceSelectorBlock
from function import getEPGsFromInterfaceProfilePath
from function import getEPGsFromPath
from function import getInterfaceProfileFromPortAndLeaf
from function import getInterfaceSelectorBlocks
from function import getPathFromLeafName
from function import getPathFromLeafPort
from function import getPathFromPolicyGroup
from function import getPortCfgFromPath
from function import getSwitchProfilesFromInterfaceProfile
from function import login
from function import unbindSwitchProfileFromInterfaceProfile
...
def main():
    ...
    try:
        ...
        logging.error(
            f"cannot parse leaf {leaf}, should be in the form: Leaf103_104"
        )
        sys.exit(1)
    ...
    if delete and fex:
        for leaf_name in leafs:
            ...
            # Get Leaf ID and POD
            leaf_path = getPathFromLeafName(
                ip=apic_ip, token=token, cookies=cookies, name=leaf_name
            )
            if not leaf_path:
            ...
            if not force and cfg["operSt"] == "up":
                confirm = input(  # nosec
                    f"Port {leaf}:{fex}:{port} is up. Continue? [no|yes]"
                )
                if confirm != "yes":
                    print("Aborting...")
                    sys.exit(0)
            ...
            if total is not 0:
                for interface_selector_block in interface_selector_blocks:
                    ...
                    selector = (
                        interface_selector_block["infraPortBlk"]["attributes"][
                            "dn"
                        ]
                        .split("/")[3]
                        .split("-")[1]
                    )
                    port_id = interface_selector_block["infraPortBlk"][
                        "attributes"
                    ]["name"]
                    total_blocks, unused = getInterfaceSelectorBlocks(
                        ip=apic_ip,
                        token=token,
                        cookies=cookies,
                        profile=fex_profile_name,
                    ...
                    if not force:
                        confirm = input(  # nosec
                            f"Removing port {port} (id={port_id}) from interface profile FEX_{leaf}:Fex{fex}/{selector}. Continue? [no|yes]"
                        )
                        if confirm != "yes":
                            print("Aborting...")
                            sys.exit(0)
                        if delete_selector:
                            confirm = input(  # nosec
                                f"Removing also FEX_{leaf}:Fex{fex}/{selector}. Continue? [no|yes]"
                            )
                            if confirm != "yes":
                                print("Aborting...")
                                sys.exit(0)
                    # Delete the interface from the Leaf Interface Profile
                    if not deleteInterfaceSelectorBlock(
                        ip=apic_ip,
                        token=token,
                        cookies=cookies,
                        name=port_id,
    ...
    if delete and not fex:
        ...
        interface_profile = getInterfaceProfileFromPortAndLeaf(
            ip=apic_ip, token=token, cookies=cookies, port=port, leaf=leaf
        )
        ...
        # Finding the path
        path = getPathFromPolicyGroup(
            ip=apic_ip, token=token, cookies=cookies, name=interface_profile
        )
        if not path:
        ...
        # Get EPGs from path
        total, leaves = getEPGsFromInterfaceProfilePath(
            ip=apic_ip,
            token=token,
            cookies=cookies,
            interface_profile_path=path,
        )
        if total > 0:
            ...
            for leaf in leaves:
                ...
                logging.error(
                    "port is used in leaf {} by: {}".format(
                        leaf_id, ", ".join(used_by)
                    )
                )
            sys.exit(1)
        ...
        for l in leafs:
            leaf_path = getPathFromLeafName(
                ip=apic_ip, token=token, cookies=cookies, name=l
            )
            if not leaf_path:
            ...
            if not force and cfg["operSt"] == "up":
                confirm = input(  # nosec
                    f"Port {l}:{fex}:{port} is up. Continue? [no|yes]"
                )
                if confirm != "yes":
                    print("Aborting...")
                    sys.exit(0)
        ...
        if total > 0:
            if not force:
                confirm = input(  # nosec
                    f"Unbinding interface profile {interface_profile} from {leaf}. Continue? [no|yes]"
                )
                if confirm != "yes":
                    print("Aborting...")
                    sys.exit(0)
            if not unbindSwitchProfileFromInterfaceProfile(
                ip=apic_ip,
                token=token,
                cookies=cookies,
                name=leaf,
        ...
        if total is 0:
            ...
            if not force:
                confirm = input(  # nosec
                    f"Deleting unused profile {interface_profile}. Continue? [no|yes]"
                )
                if confirm != "yes":
                    print("Aborting...")
                    sys.exit(0)
            if not deleteInterfaceProfile(
                ip=apic_ip,
                token=token,
                cookies=cookies,
                name=interface_profile,
