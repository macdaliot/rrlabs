...
def main():
    ...
    total, switch_profiles = getSwitchProfiles(
        ip=apic_ip, token=token, cookies=cookies, name=leaf_profile
    )
    if total is 0:
    ...
    total, interface_profiles = getInterfaceProfiles(
        ip=apic_ip, token=token, cookies=cookies, name=device_name
    )
    if total is 0 or force:
    ...
    if total > 1:
        ...
        if not bindSwitchProfileToInterfaceProfile(
            ...
            cookies=cookies,
            name=leaf_profile,
            interface_profile=device_name,
        ):
            logging.error(
                f"failed to bind switch profile {leaf_profile} to interface profile {device_name}"
            )
            sys.exit(1)
