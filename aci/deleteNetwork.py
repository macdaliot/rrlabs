...
import sys
import yaml
from functions import deleteBD
from functions import deleteEPG
from functions import deleteSubnet
from functions import getPathFromEPG
from functions import getSubnets
from functions import login
from functions import unbindBDtoL3Out
...
def main():
    ...
    if only_subnet:
        ...
        total, subnets = getSubnets(
            ip=apic_ip, token=token, cookies=cookies, tenant=tenant, bd=name
        )
        # Unbinding the L3Out (mandatory to avoid blackhole)
        if not unbindBDtoL3Out(
            ip=apic_ip,
            token=token,
            cookies=cookies,
            name=name,
    ...
    if not force:
        confirm = input(f"Deleting EPG {name}. Continue? [no|yes]")  # nosec
        if confirm != "yes":
            print("Aborting...")
            sys.exit(0)
    ...
    if not force:
        confirm = input(f"Deleting BD {name}. Continue? [no|yes]")  # nosec
        if confirm != "yes":
            print("Aborting...")
            sys.exit(0)

    if not deleteBD(
        ip=apic_ip, token=token, cookies=cookies, tenant=tenant, name=name
    ):
